                        ...
                        <td colspan="2">
                            <c:if test="${isLogin}">
                                ...
                                <c:if test="${memberId == comment.userEmail}">
                                    <button id="${cs}_${comment.number}_update_btn" onclick="updateComment('${cs}', '${comment.number}')" >modify</button>
                                    <button id="${cs}_${comment.number}_delete_btn" onclick="deleteComment('${cs}', '${comment.number}', '${param.boardNumber}')" >delete</button>
                                </c:if>
                            </c:if>
                        </td>
                    </tr>
...
<div id="comment_paging">
        <c:if test="${pagingVO.beginPage > 10}">
            <button onclick="showComment(${pagingVO.beginPage-1}, '${cs}')">Previous</button>
        </c:if>
        ...
        <c:if test="${pagingVO.endPage < pagingVO.totalPageCount}">
            <button onclick="showComment(${pagingVO.endPage+1}, '${cs}')">Next</button>
        </c:if>
</div>
...
<c:if test="${isLogin}">
        ...
        <table style="border: 2px solid; margin: 30px 10px; width: 50%;">
            <tr>
                <td>
                    ...
                    <button id="${cs}_ajaxform_cancle" onclick="toggleWrite('${cs}')" >cancel</button>
                    <button id="${cs}_ajaxform_submit" onclick="ajaxForm('${cs}')" >submit</button>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div id="${cs}_write_btn">
        <button onclick="toggleWrite('${cs}')">write</button>
</div>
</c:if>